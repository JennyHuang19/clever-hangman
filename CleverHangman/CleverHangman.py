...
def getNewWordList(currTemplate, letterGuess, wordList, DEBUG):
    ...
    sortedKeys = sorted(templateDict) #want templates to be printed alphabetically.
...
def runGame(filename):
    ...
    #print(newWordList)
#   Choose a random word from the new wordList.
    word = random.choice(newWordList)
    
    while missesLeft >= 1 and "_" in currTemplate: #is guessedLetter different from letterGuess?
        ...
        L = len(newWordList)
        #redefine wordlist as the newWordList
        wordList = newWordList
        #should the secret word be chosen here?
        ...
        #create the updated template (hangmanWord) after a guessed letter.
        hangmanWord = tuple[0] #string
        currTemplate = hangmanWord
        newWordList = tuple[1]
        ...
        Lst = processUserGuessClever(letterGuess, list(hangmanWord), missesLeft) #the hangmanWord should be a list of strings.
        missesLeft = Lst[0]
